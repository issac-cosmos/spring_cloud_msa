            ...
            <v-col>
                <v-form>
                    <v-row>
                        ...
                        <v-col>
                            <v-text-field style="width: 300px ; height: 30px;"
...
export default{
    ...
    methods: {
        ...
        async loadData(){
            try{
                // if(this.isLoading || this.isLastPage)return; 없어도될듯?
                this.isLoading=true;
                // params 라는 키워드를 사용하면 아래 데이터가 ?size=xxx&page=yyy 형태의 파라미터 형식으로 전송
                let params = {
                    size: this.pageSize,
                ...
                const additionalData = response.data.content.map(p => ({...p, productCount:0}));
        ...
        scrollPagenation(){
            // 화면높이 + 스크롤로 이동한거리 > 전체화면 - n(내가 원하는 길이)가 성립되면 추가 데이터 로드 
        ...
        async createOrder(){
            // selected된 product만 filtering 해줘야함
            // [{productId:1, productCount:3},{productId:3, productCount:4}]
            const orderProductList = Object.keys(this.selected).filter(key=>this.selected[key]===true)
            .map(key =>{
                const product = this.productList.filter(p=>p.productCount>0).find(p=>p.id==key);
                if(product){
            ...
            const yesOrNo = confirm(`${totalCount}개의 상품을 주문하시겠습니가?`);
            if(!yesOrNo){
